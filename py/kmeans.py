...
#plt.scatter(centers[:, 0], centers[:, 1], c=weightcount, s=5, cmap='Spectral')
...
workbook1 = xlsw.Workbook('kcluster.xlsx')
data = workbook1.add_worksheet('data')
...
workbook2 = xlsw.Workbook('distances.xlsx')
dist = workbook2.add_worksheet('distances')
...
for i in range(1, n):
    for j in range(1, n):
        orig = (centers[i, 0], centers[i, 1])
        dest = (centers[j, 0], centers[j, 1])
